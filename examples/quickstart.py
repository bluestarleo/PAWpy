...
def main():
    with PAWService(
        ...
        scope="paw",
        database="Global FPA",   # default TM1 database for paw.tm1()
        verify=True,
    ) as paw:

        # --- Content: list books in a folder -------------------------------
        for book in paw.books.get_all("/shared/FP&A"):
            print(book.name, "->", paw.books.get_embed_url(book.path))

        # --- Content: generic asset CRUD -----------------------------------
        folder = paw.content.create_folder(name="Reports 2026", path="/shared/FP&A")
        ...
        print(paw.ui.book_url("/shared/FP&A/Monthly Report", embed=True))
        print(paw.ui.cube_viewer_url("Global FPA", "Revenue Cube", view="Monthly View"))
        print(paw.ui.dimension_editor_url("Global FPA", "h_accounts"))
